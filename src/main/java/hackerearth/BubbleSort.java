...
/**
 * Sorting Algorithms are concepts that every competitive programmer must know. Sorting algorithms
 * can be used for collections of numbers, strings, characters, or a structure of any of these
 * types.
 *
 ...
 * elements are not swapped. However, when 5 is compared with 5 > 2, and these elements are in
 * descending order. Therefore, 5 and 2
 *
 * <p>are swapped.
 *
 * <p>Now the array is
...
class BubbleSort {
  ...
  public BubbleSort(InputReader inputReader, OutputWriter outputWriter) {
  ...
  List<Integer> calculate() throws IOException {
    ...
    swapCount = bubbleSort(sizeOfArray, array, swapCount);

    if (outputWriter != null) {

      outputWriter.println(swapCount);
    }

    return array;
  }

  public int bubbleSort(int sizeOfArray, List<Integer> array, int swapCount) {
